    ...
    <div class="main">
      <div class="from">
        <div class="list vux-1px-b">
          <label for="">{{$t('新手机号')}}：</label>
          <input type="text" :placeholder="$t('请输入新手机号')" v-model='mobile'>
        </div>
        <div class="list vux-1px-b">
          <label for="">{{$t('短信验证')}}：</label>
          <input type="text" :placeholder="$t('新手机短信验证码')" v-model='code'>
          <button @click='checksms'>{{codeName}}</button>
        </div>
        <div class="btn">
          <button @click='modSubmit'>{{$t('确 认')}}</button>
        </div>
      </div>
    </div>
  </div>
    ...
    methods: {
      ...
      checksms() {
        if (this.codeBtn) {
          ...
          that.codeBtn = false
          var play = null
          var data1 = {mobile: that.mobile, type: 10015,}
      ...
      modSubmit() {
        var that = this
        var url = that.$inter + 'Member/editmobile'
        var data = {
          mobile: that.mobile,
          code: that.code,
        ...
        that.$axios.post(url, data).then(function (response) {
          if (response.data.error != 0) {
            ...
            that.$Toast(response.data.message)
            that.eosInfo.mobile = that.mobile.substr(0, 7)+'****'
