<template>
  <div id="cast">
    <Header>
      <template slot='conter'>{{$t('修改登录密码')}}</template>
    </Header>
    <div class="main">
      <div class="from">
        <div class="list vux-1px-b">
          <label for="">{{$t('登录密码')}}：</label>
          <input type="password" :placeholder="$t('请输入登录密码')" v-model='old_password'>
        </div>
        <div class="list vux-1px-b">
          <label for="">{{$t('新密码')}}：</label>
          <input type="password" :placeholder="$t('请输入新的登录密码')" v-model='login_password1'>
        </div>
        <div class="list vux-1px-b">
          <label for="">{{$t('新密码')}}：</label>
          <input type="password" :placeholder="$t('请再次输入新的登录密码')" v-model='login_password2'>
        </div>
        <div class="list vux-1px-b">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      modSubmit() {
        var that = this
        var url = that.$inter + 'Login/editLoginPassword'
        var data = {
