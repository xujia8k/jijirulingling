<template>
  <div id="cast">
    <Header>
      <template slot='conter'>{{$t('我的SEOS转出地址')}}</template>
    </Header>
    <div class="main">
      <div class="inve_type vux-1px-b">
        <h6>{{$t('请输入你的SEOS转出地址')}}</h6>
        <input type="text" :placeholder="$t('请输入SEOS转出地址')" v-model='address'>
      </div>
      <div class="from">
  ...
  export default {
    ...
    created() {
      ...
      if (localStorage.getItem('eosInfo')) {
        ...
        this.address = this.eosInfo.seos_address
      }
    },
    mounted() {
    },
        ...
        if (this.switchBtn) {
          ...
          that.$axios.post(url, data).then(function (response) {
            if (response.data.error != 0) {
              ...
              that.$Toast(response.data.message)
              that.eosInfo.eos_address = that.address
